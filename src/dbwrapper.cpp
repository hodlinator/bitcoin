    ...
    : m_db_context{std::make_unique<LevelDBContext>()}, m_name{fs::PathToString(params.path.stem())}, m_path{params.path}, m_is_memory{params.memory_only}
{
    ...
    {
        std::vector<uint8_t> obfuscation_key(Obfuscation::SIZE_BYTES); // Zero-initialized is needed for unobfuscated Read
        const bool key_missing{!Read(OBFUSCATE_KEY_KEY, obfuscation_key)};
        if (key_missing && params.obfuscate && IsEmpty()) {
            ...
            obfuscation_key = std::move(new_key);

            LogPrintf("Wrote new obfuscate key for %s: %s\n", fs::PathToString(params.path), HexStr(obfuscation_key));
        }
        LogPrintf("Using obfuscation key for %s: %s\n", fs::PathToString(params.path), HexStr(obfuscation_key));
        assert(obfuscation_key.size() == Obfuscation::SIZE_BYTES);
        m_obfuscation = {std::span{obfuscation_key}.first<Obfuscation::SIZE_BYTES>()};
    }
}
