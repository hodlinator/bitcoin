...
HeadersSyncState::HeadersSyncState(NodeId id, const Consensus::Params& consensus_params,
    ...
    m_presync({.chain_work = chain_start->nChainWork,
               ...
               .headers_cache = {.chain_work = chain_start->nChainWork,
                                 .cap = ComputeHeadersCacheSize<decltype(m_presync.headers_cache.data)>(cache_bytes, chain_start, consensus_params)}})
{
    ...
    m_max_commitments = std::max<int64_t>(0, (6*(Ticks<std::chrono::seconds>(NodeClock::now() - NodeSeconds{std::chrono::seconds{chain_start->GetMedianTimePast()}}) + MAX_FUTURE_BLOCK_TIME) - m_presync.headers_cache.cap) / HEADER_COMMITMENT_PERIOD);
 ...
 * SaltedTxidHasher for another sync. It also ensures that we don't somehow
...
bool HeadersSyncState::ValidateAndStoreRedownloadedHeader(const CBlockHeader& header)
{
    ...
    if (!m_redownload.process_all_remaining_headers && next_height % HEADER_COMMITMENT_PERIOD == m_commit_offset) {
...
std::vector<CBlockHeader> HeadersSyncState::PopHeadersReadyForAcceptance()
{
    ...
    while (m_redownload.headers.size() > REDOWNLOAD_BUFFER_SIZE ||
