...
using util::Split;
std::optional<std::string> HTTPHeaders::FindFirst(std::string_view key) const
{
...
void HTTPRequest::WriteReply(HTTPStatusCode status, std::span<const std::byte> reply_body)
{
    ...
    res.m_status = status;
    res.m_reason = std::ranges::find_if(g_http_reasons, [&] (auto& pair) { return pair.first == status; })->second;
...
const CService& HTTPRequest::GetPeer() const
{
...
std::pair<bool, std::string> HTTPRequest::GetHeader(std::string_view hdr) const
{
    ...
    for (const auto& [sock, events] : io_readiness.events_per_sock) {
        ...
        if (send_ready) {
            ...
            if (!client->MaybeSendBytesFromBuffer()) {
                ...
                // Send was successful, reset idle timeout
...
void HTTPServer::SocketHandlerListening(const Sock::EventsPerSock& events_per_sock)
{
    for (const auto& sock : m_listen) {
        ...
        if (it != events_per_sock.end() && it->second.occurred & Sock::RECV) {
            if (auto sock_accepted{AcceptConnection(*sock)}) {
                NewSockAccepted(std::move(sock_accepted->first), sock_accepted->second);
            }
        }
    }
}
...
void HTTPServer::DisconnectClients()
{
    ...
    size_t erased = std::erase_if(m_connected,
                                  [&](auto& client) {
                                        ...
                                        bool is_idle = (now - client->m_idle_since > m_rpcservertimeout) && !client->m_req_busy;
...
bool InitHTTPServer()
{
    ...
    for (std::vector<std::pair<std::string, uint16_t>>::iterator i = endpoints.begin(); i != endpoints.end(); ++i) {
        ...
        if (addr) {
            ...
            LogWarning("Could not bind RPC on address %s port %i: Address lookup failed.", i->first, i->second);
        }
    }
