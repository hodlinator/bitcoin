...
public:
    ...
    const std::string& GetURI() const { return m_target; }
    ...
    const std::string& ReadBody() const { return m_body; }
...
std::optional<std::string> GetQueryParameterFromUri(const std::string& uri, std::string_view key);
...
public:
    ...
    //! `m_keep_alive=true` can be overriden `by HTTPServer.m_disconnect_all` (we disconnect).
    ...
    //! Might be set in a worker thread or in the I/O thread. Overrides all other disconnect flags.
