    ...
    const btck_ChainstateManagerOptions* chainman_opts)
{
    ...
    try {
        ...
        if (!load_result || IsInterrupted(*load_result)) {
            LogError("Failed to load chain state from your data directory: %s", load_result.error().second.original);
            return nullptr;
        }
        auto verify_result{node::VerifyLoadedChainstate(*chainman, chainstate_load_opts)};
        if (!verify_result || IsInterrupted(*verify_result)) {
            LogError("Failed to verify loaded chain state from your datadir: %s", verify_result.error().second.original);
            return nullptr;
        }
