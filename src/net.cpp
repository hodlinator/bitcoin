...
std::unique_ptr<CNode> CConnman::ConnectNode(CAddress addrConnect, const char *pszDest, bool fCountFailure, ConnectionType conn_type, bool use_v2transport)
{
                                            ...
                                            const CService& addr)
{
    ...
    m_msgproc->InitializeNode(*pnode, local_services);
    {
        ...
        TRACEPOINT(net, inbound_connection,
            m_nodes.back()->GetId(),
            m_nodes.back()->m_addr_name.c_str(),
            m_nodes.back()->ConnectionTypeAsString().c_str(),
            m_nodes.back()->ConnectedThroughNetwork(),
...
bool CConnman::AddConnection(const std::string& address, ConnectionType conn_type, bool use_v2transport = false)
{
    ...
    const int existing_connections = WITH_LOCK(m_nodes_mutex,
        return std::ranges::count_if(m_nodes, [conn_type](auto& node) { return node->m_conn_type == conn_type; }););
...
void CConnman::OpenNetworkConnection(const CAddress& addrConnect, bool fCountFailure, CSemaphoreGrant&& grant_outbound, const char *pszDest, ConnectionType conn_type, bool use_v2transport)
{
    ...
    std::unique_ptr<CNode> pnode = ConnectNode(addrConnect, pszDest, fCountFailure, conn_type, use_v2transport);
    ...
    {
        LOCK(m_nodes_mutex);
        const CNode* node_ptr = pnode.get();
        m_nodes.emplace_back(std::move(pnode));

        // update connection count by network
        if (node_ptr->IsManualOrFullOutboundConn()) ++m_network_conn_counts[node_ptr->addr.GetNetwork()];

        TRACEPOINT(net, outbound_connection,
            node_ptr->GetId(),
            node_ptr->m_addr_name.c_str(),
            node_ptr->ConnectionTypeAsString().c_str(),
            node_ptr->ConnectedThroughNetwork(),
