    ...
    struct PeerDoSInfo {
        ...
        * limits are exceeded, it must be that there is a peer whose DoS score > 1. */
        FeeFrac GetDosScore(TxOrphanage::Count max_peer_latency_score, TxOrphanage::Usage max_peer_memory) const
        {
            ...
            const FeeFrac latency_score(Amount::From(static_cast<Amount::inner_type>(m_total_latency_score)), max_peer_latency_score);
            const FeeFrac mem_score(Amount::From(m_total_usage), max_peer_memory);
