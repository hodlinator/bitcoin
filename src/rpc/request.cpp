...
static std::optional<fs::path> GetAuthCookieFile(bool temp=false)
{
    fs::path arg = gArgs.GetPathArg("-rpccookiefile", COOKIEAUTH_FILE);
    if (arg.empty()) {
        return std::nullopt; // -norpccookiefile was specified
    }
    if (temp) {
        arg += ".tmp";
    }
...
bool GenerateAuthCookie(std::string* cookie_out, std::optional<fs::perms> cookie_perms)
{
     ...
     */
    std::ofstream file;
    auto filepath_tmp = GetAuthCookieFile(true);
    if (!filepath_tmp) {
    ...
    // .value() - if the previous GetAuthCookieFile() succeeds, this should too.
    fs::path filepath = GetAuthCookieFile(false).value();
...
bool GetAuthCookie(std::string *cookie_out)
{
    std::ifstream file;
    std::string cookie;
    auto filepath = GetAuthCookieFile();
    if (!filepath) {
...
void DeleteAuthCookie()
{
    try {
        if (g_generated_cookie) {
            ...
            if (auto filepath{GetAuthCookieFile()})
                fs::remove(*filepath);
        }
