...
struct Node {
    ...
    /* Destroy the subexpressions iteratively to avoid a stack-overflow due to recursive calls
    ...
    Node<Key> Clone() const
    {
        ...
        auto upfn = [](const Node& node, Span<Node> children) {
            Node ret{internal::DeferredSubsClone{}, node};
            ret.subs.reserve(children.size());
            for (auto& c : children) {
                ret.subs.emplace_back(std::move(c));
            }
            return ret;
        };
    ...
    // Copying would have recursive behavior via subs, use Clone() instead.
    Node(const Node&) = delete;
    Node& operator=(const Node&) = delete;

private:
    // Private constructor used for cloning.
    Node(internal::DeferredSubsClone&&, const Node& other)
        : fragment{other.fragment}, k{other.k}, keys{other.keys}, data{other.data}, subs{}, m_script_ctx{other.m_script_ctx}, ops{other.ops}, ss{other.ss}, ws{other.ws}, typ{other.typ}, scriptlen{other.scriptlen}, has_duplicate_keys{other.has_duplicate_keys} {}
