...
public:
    ...
    void read(std::span<std::byte> dst)
    {
        if (const auto available{std::min(dst.size(), m_buf_used.size())}) {
            std::copy_n(m_buf_used.begin(), available, dst.begin());
            m_buf_used = m_buf_used.subspan(available);
            dst = dst.subspan(available);
        }
        if (dst.size()) {
            assert(m_buf_used.empty());
            ...
            if (dst.size() > m_buf.size()) {
                ...
                m_buf_used = {m_buf.begin(), m_src.detail_fread(m_buf)};

                if (m_buf_used.size() < dst.size()) {
                    throw std::ios_base::failure(m_src.feof() ? "BufferedReader::read: end of file" : "BufferedReader::read: fread failed");
                }

                std::copy_n(m_buf_used.begin(), dst.size(), dst.begin());
                m_buf_used = m_buf_used.subspan(dst.size());
            }
        }
    }
