...
BOOST_FIXTURE_TEST_CASE(invalidate_block, TestChain100Setup)
{
    ...
    BlockValidationState state{m_node.chainman->ActiveChainstate().InvalidateBlock(tip_to_invalidate)};
    ...
    (void)m_node.chainman->ActiveChainstate().InvalidateBlock(orig_tip);
