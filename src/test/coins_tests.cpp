...
BOOST_FIXTURE_TEST_CASE(updatecoins_simulation_test, UpdateTest)
{
    ...
    for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
        ...
        if (randiter % 20 < 19) {
            ...
            tx.vin.resize(1);
            tx.vout.resize(1);
            tx.vout[0].nValue = CAmount{i}; //Keep txs unique unless intended to duplicate
...
constexpr CAmountUnchecked SPENT {-1};
constexpr CAmountUnchecked ABSENT{-2};
...
struct CoinEntry {
    ...
    const CAmountUnchecked value;
    const State state;

    constexpr CoinEntry(const CAmountUnchecked v, const State s) : value{v}, state{s} {}
...
static void SetCoinsValue(const CAmountUnchecked value, Coin& coin)
{
    assert(value != ABSENT);
    coin.Clear();
    assert(coin.IsSpent());
    if (value != SPENT) {
        coin.out.nValue = value.AssertValid();
        coin.nHeight = 1;
        assert(!coin.IsSpent());
    }
}
...
{
public:
    SingleEntryCacheTest(const CAmountUnchecked base_value, const MaybeCoin& cache_coin)
    {
...
static void CheckAccessCoin(const CAmountUnchecked base_value, const MaybeCoin& cache_coin, const MaybeCoin& expected)
{
...
BOOST_AUTO_TEST_CASE(ccoins_access)
{
     ...
     *                  Base        Cache               Expected
     */
    for (auto base_value : {ABSENT, SPENT, CAmountUnchecked{VALUE1}}) {
...
static void CheckSpendCoins(const CAmountUnchecked base_value, const MaybeCoin& cache_coin, const MaybeCoin& expected)
{
    SingleEntryCacheTest test{base_value, cache_coin};
...
static void CheckAddCoin(const CAmountUnchecked base_value, const MaybeCoin& cache_coin, const CAmount modify_value, const CoinOrError& expected, const bool coinbase)
{
    SingleEntryCacheTest test{base_value, cache_coin};
...
{
    Coin coin;
    coin.out.nValue = CAmount{m_rng.rand32()};
    ...
    bool do_erasing_flush)
{
    ...
    outp = COutPoint(txid, 0);
    coin = MakeCoin();
    CAmount coin_val = coin.out.nValue.AssertValid();
    BOOST_CHECK(!base.HaveCoin(outp));
    BOOST_CHECK(!all_caches[0]->HaveCoin(outp));
    BOOST_CHECK(!all_caches[1]->HaveCoin(outp));
...
BOOST_AUTO_TEST_CASE(ccoins_addcoin_exception_keeps_usage_balanced)
{
    ...
    const Coin coin1{CTxOut{CAmount{m_rng.randrange(10)}, CScript{} << m_rng.randbytes(CScriptBase::STATIC_SIZE + 1)}, 1, false};
    cache.AddCoin(outpoint, Coin{coin1}, /*possible_overwrite=*/false);
    cache.SelfTest();

    const Coin coin2{CTxOut{CAmount{m_rng.randrange(20)}, CScript{} << m_rng.randbytes(CScriptBase::STATIC_SIZE + 2)}, 2, false};
...
BOOST_AUTO_TEST_CASE(ccoins_reset_guard)
{
    ...
    const Coin coin{CTxOut{CAmount{m_rng.randrange(10)}, CScript{} << m_rng.randbytes(CScriptBase::STATIC_SIZE + 1)}, 1, false};
...
BOOST_AUTO_TEST_CASE(ccoins_peekcoin)
{
    ...
    const COutPoint outpoint{Txid::FromUint256(m_rng.rand256()), m_rng.rand32()};
    const Coin coin{CTxOut{CAmount{m_rng.randrange(10)}, CScript{}}, 1, false};
    {
