...
struct CoinEntry {
    enum State { CLEAN, DIRTY, FRESH, DIRTY_FRESH };
    ...
    constexpr bool IsDirtyFresh() const { return state == DIRTY_FRESH; }
    constexpr bool IsDirty() const { return state == DIRTY || IsDirtyFresh(); }
    constexpr bool IsFresh() const { return state == FRESH || IsDirtyFresh(); }

    static constexpr State ToState(bool is_dirty, bool is_fresh) {
        if (is_dirty && is_fresh) return DIRTY_FRESH;
...
constexpr MaybeCoin SPENT_DIRTY       {{SPENT,  CoinEntry::DIRTY}};
constexpr MaybeCoin SPENT_DIRTY_FRESH {{SPENT,  CoinEntry::DIRTY_FRESH}};
constexpr MaybeCoin SPENT_FRESH       {{SPENT,  CoinEntry::FRESH}};
constexpr MaybeCoin SPENT_CLEAN       {{SPENT,  CoinEntry::CLEAN}};
constexpr MaybeCoin VALUE1_DIRTY      {{VALUE1, CoinEntry::DIRTY}};
constexpr MaybeCoin VALUE1_DIRTY_FRESH{{VALUE1, CoinEntry::DIRTY_FRESH}};
constexpr MaybeCoin VALUE1_FRESH      {{VALUE1, CoinEntry::FRESH}};
constexpr MaybeCoin VALUE1_CLEAN      {{VALUE1, CoinEntry::CLEAN}};
constexpr MaybeCoin VALUE2_DIRTY      {{VALUE2, CoinEntry::DIRTY}};
constexpr MaybeCoin VALUE2_DIRTY_FRESH{{VALUE2, CoinEntry::DIRTY_FRESH}};
constexpr MaybeCoin VALUE2_FRESH      {{VALUE2, CoinEntry::FRESH}};
constexpr MaybeCoin VALUE2_CLEAN      {{VALUE2, CoinEntry::CLEAN}};
constexpr MaybeCoin VALUE3_DIRTY      {{VALUE3, CoinEntry::DIRTY}};
constexpr MaybeCoin VALUE3_DIRTY_FRESH{{VALUE3, CoinEntry::DIRTY_FRESH}};
...
public:
    ...
    {
        auto base_cache_coin{base_value == ABSENT ? MaybeCoin{} : CoinEntry{base_value, CoinEntry::DIRTY}};
...
BOOST_AUTO_TEST_CASE(ccoins_add)
{
    ...
    for (auto base_value : {ABSENT, SPENT, VALUE1}) {
        CheckAddCoin(base_value, std::nullopt,       VALUE3, VALUE3_DIRTY_FRESH,   false);
        CheckAddCoin(base_value, std::nullopt,       VALUE3, VALUE3_DIRTY,         true );
...
BOOST_AUTO_TEST_CASE(ccoins_write)
{
     ...
     */
    CheckWriteCoins(std::nullopt,       std::nullopt,       std::nullopt       );
    CheckWriteCoins(std::nullopt,       SPENT_DIRTY,        SPENT_DIRTY        );
    CheckWriteCoins(std::nullopt,       SPENT_DIRTY_FRESH,  std::nullopt       );
    CheckWriteCoins(std::nullopt,       VALUE2_DIRTY,       VALUE2_DIRTY       );
    CheckWriteCoins(std::nullopt,       VALUE2_DIRTY_FRESH, VALUE2_DIRTY_FRESH );
    CheckWriteCoins(SPENT_CLEAN,        std::nullopt,       SPENT_CLEAN        );
    CheckWriteCoins(SPENT_FRESH,        std::nullopt,       SPENT_FRESH        );
    CheckWriteCoins(SPENT_DIRTY,        std::nullopt,       SPENT_DIRTY        );
    CheckWriteCoins(SPENT_DIRTY_FRESH,  std::nullopt,       SPENT_DIRTY_FRESH  );

    CheckWriteCoins(SPENT_CLEAN,        SPENT_DIRTY,        SPENT_DIRTY        );
    CheckWriteCoins(SPENT_CLEAN,        SPENT_DIRTY_FRESH,  SPENT_DIRTY        );
    CheckWriteCoins(SPENT_FRESH,        SPENT_DIRTY,        std::nullopt       );
    CheckWriteCoins(SPENT_FRESH,        SPENT_DIRTY_FRESH,  std::nullopt       );
    CheckWriteCoins(SPENT_DIRTY,        SPENT_DIRTY,        SPENT_DIRTY        );
    CheckWriteCoins(SPENT_DIRTY,        SPENT_DIRTY_FRESH,  SPENT_DIRTY        );
    CheckWriteCoins(SPENT_DIRTY_FRESH,  SPENT_DIRTY,        std::nullopt       );
    CheckWriteCoins(SPENT_DIRTY_FRESH,  SPENT_DIRTY_FRESH,  std::nullopt       );
    ...
    CheckWriteCoins(VALUE1_CLEAN,       std::nullopt,       VALUE1_CLEAN       );
    CheckWriteCoins(VALUE1_FRESH,       std::nullopt,       VALUE1_FRESH       );
    CheckWriteCoins(VALUE1_DIRTY,       std::nullopt,       VALUE1_DIRTY       );
    CheckWriteCoins(VALUE1_DIRTY_FRESH, std::nullopt,       VALUE1_DIRTY_FRESH );
    CheckWriteCoins(VALUE1_CLEAN,       SPENT_DIRTY,        SPENT_DIRTY        );
    CheckWriteCoins(VALUE1_CLEAN,       SPENT_DIRTY_FRESH,  EX_FRESH_MISAPPLIED);
    CheckWriteCoins(VALUE1_FRESH,       SPENT_DIRTY,        std::nullopt       );
    ...
    CheckWriteCoins(VALUE1_DIRTY_FRESH, SPENT_DIRTY,        std::nullopt       );
    ...
    bool do_erasing_flush)
{
    ...
    BOOST_CHECK_EQUAL(GetCoinsMapEntry(view->map(), outp), CoinEntry(coin.out.nValue, CoinEntry::DIRTY_FRESH));
    ...
    //
    BOOST_CHECK_EQUAL(GetCoinsMapEntry(view->map(), outp), CoinEntry(coin.out.nValue, CoinEntry::CLEAN)); // State should have been wiped.
    ...
    if (do_erasing_flush) {
        ...
        BOOST_CHECK_EQUAL(GetCoinsMapEntry(view->map(), outp), CoinEntry(coin.out.nValue, CoinEntry::CLEAN));
    }
    ...
    BOOST_CHECK_EQUAL(GetCoinsMapEntry(all_caches[0]->map(), outp), CoinEntry(coin_val, CoinEntry::DIRTY_FRESH));
