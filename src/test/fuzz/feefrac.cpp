...
FUZZ_TARGET(feefrac)
{
    ...
    auto cmp_mul = FeeFrac::Mul(Amount::From(f1), s2) <=> FeeFrac::Mul(Amount::From(f2), s1);
    assert(cmp_mul == cmp_feerate);

    // Same, but using FeeFrac::MulFallback.
    auto cmp_fallback = FeeFrac::MulFallback(Amount::From(f1), s2) <=> FeeFrac::MulFallback(Amount::From(f2), s1);
...
FUZZ_TARGET(feefrac_mul_div)
{
    ...
    auto res = FeeFrac::Div(FeeFrac::Mul(Amount::From(mul64), mul32), div, round_down);
    assert(res == 0 || (res < 0) == is_negative);
    assert(Abs256(res) == quot_abs);

    // Verify the behavior of FeeFrac::MulFallback + FeeFrac::DivFallback.
    auto res_fallback = FeeFrac::DivFallback(FeeFrac::MulFallback(Amount::From(mul64), mul32), div, round_down);
    ...
    if (mul32 >= 0) {
        auto res_fee = round_down ?
            FeeFrac{Amount::From(mul64), div}.EvaluateFeeDown(mul32) :
            FeeFrac{Amount::From(mul64), div}.EvaluateFeeUp(mul32);
        ...
        if (mul64 < std::numeric_limits<int64_t>::max() / 1000 &&
            ...
            CFeeRate feerate(Amount::From(mul64), div);
            Amount feerate_fee{feerate.GetFee(mul32)};
            Amount allowed_gap{Amount::From(static_cast<int64_t>(mul32 / 1000 + 3 + round_down))};
