...
std::optional<Node> GenNode(MsCtx script_ctx, F ConsumeNode, Type root_type, bool strict_valid = false) {
    ...
    while (!todo.empty()) {
        ...
        if (!todo.back().second) {
            ...
            if (node->GetFragment() == Fragment::WRAP_V && node->GetSubs()[0].GetType() << "x"_mst) {
                ops += 1;
                scriptsize += 1;
            }
...
void TestNode(const MsCtx script_ctx, const std::optional<Node> node, FuzzedDataProvider& provider)
{
    ...
    bool satisfiable = node->IsSatisfiable([&](const Node& node) -> bool {
        ...
        case Fragment::AFTER:
            return node.GetK() & 1;
        case Fragment::SHA256:
            return TEST_DATA.sha256_preimages.count(node.GetData());
        case Fragment::HASH256:
            return TEST_DATA.hash256_preimages.count(node.GetData());
        case Fragment::RIPEMD160:
            return TEST_DATA.ripemd160_preimages.count(node.GetData());
        case Fragment::HASH160:
            return TEST_DATA.hash160_preimages.count(node.GetData());
        default:
            assert(false);
        }
        return false;
