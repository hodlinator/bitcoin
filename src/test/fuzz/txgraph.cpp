...
struct SimTxGraph
{
    ...
    void SetTransactionFee(TxGraph::Ref* ref, Amount fee)
    {
        auto pos = Find(ref);
    ...
    LIMITED_WHILE(provider.remaining_bytes() > 0, 200) {
        ...
        while (true) {
            if ((block_builders.empty() || sims.size() > 1) && top_sim.GetTransactionCount() < SimTxGraph::MAX_TRANSACTIONS && command-- == 0) {
                ...
                FeePerWeight feerate{Amount::From(fee), size};
                ...
                auto ref = pick_fn();
                real->SetTransactionFee(*ref, Amount::From(fee));
                for (auto& sim : sims) {
                    sim.SetTransactionFee(ref, Amount::From(fee));
                }
                break;
            } else if (command-- == 0) {
                // GetTransactionCount.
