...
BOOST_AUTO_TEST_CASE(test_query_parameters_new_behavior)
{
    ...
    BOOST_CHECK_EQUAL(http_bitcoin::GetQueryParameterFromUri(uri, "p1").value(), "v1");
    BOOST_CHECK_EQUAL(http_bitcoin::GetQueryParameterFromUri(uri, "p2").value(), "v2%");
    ...
    BOOST_CHECK_EQUAL(http_bitcoin::GetQueryParameterFromUri(uri, "p1").value(), "v1");
    BOOST_CHECK_EQUAL(http_bitcoin::GetQueryParameterFromUri(uri, "p2").value(), "100%");
}
...
BOOST_AUTO_TEST_CASE(http_headers_tests)
{
    ...
    }
    {
        ...
        std::span missing_a_colon{StringToBytes("key value\n")};
        ...
        std::span missing_a_key{StringToBytes(":value\n")};
        ...
        // Fixed
        std::span fixed_headers{StringToBytes("key:value\n")};
...
BOOST_AUTO_TEST_CASE(http_response_tests)
{
    ...
    res.m_reason = std::ranges::find_if(g_http_reasons, [&] (auto& pair) { return pair.first == res.m_status; })->second;
    res.m_body = StringToBuffer(R"({"result":865793,"error":null,"id":null})");
...
BOOST_AUTO_TEST_CASE(http_request_tests)
{
    ...
    }
    {
        ...
        const std::span too_short_request_line{StringToBytes("GET/HTTP/1.0\r\nHost: 127.0.0.1\r\n\r\n")};
        HTTPRequest req;
    ...
    }
    {
        ...
        std::span malformed_request_line{StringToBytes("GET / HTTP / 1.0\r\nHost: 127.0.0.1\r\n\r\n")};
        HTTPRequest req;
    ...
    }
    {
        ...
        std::span malformed_request_line{StringToBytes("GET / HTTP1.0\r\nHost: 127.0.0.1\r\n\r\n")};
        HTTPRequest req;
        LineReader reader(malformed_request_line, MAX_HEADERS_SIZE);
        BOOST_CHECK_EXCEPTION(req.LoadControlData(reader), std::runtime_error, HasReason{"HTTP request line too short"});
    }
    {
        ...
        std::span malformed_request_line{StringToBytes("GET / HTTP/11\r\nHost: 127.0.0.1\r\n\r\n")};
        HTTPRequest req;
        LineReader reader(malformed_request_line, MAX_HEADERS_SIZE);
        BOOST_CHECK_EXCEPTION(req.LoadControlData(reader), std::runtime_error, HasReason{"HTTP request line too short"});
    }
    {
        ...
        std::span malformed_request_line{StringToBytes("GET / HTTP/1.x\r\nHost: 127.0.0.1\r\n\r\n")};
        HTTPRequest req;
        LineReader reader(malformed_request_line, MAX_HEADERS_SIZE);
    ...
    }
    {
        ...
        std::span malformed_request_line{StringToBytes("GET / HTTP/2.0\r\nHost: 127.0.0.1\r\n\r\n")};
        HTTPRequest req;
        LineReader reader(malformed_request_line, MAX_HEADERS_SIZE);
        BOOST_CHECK_EXCEPTION(req.LoadControlData(reader), std::runtime_error, HasReason{"HTTP bad version"});
    }
    ...
    {
        std::span ok_request_line{StringToBytes("GET / HTTP/1.0\r\nHost: 127.0.0.1\r\n\r\n")};
        HTTPRequest req;
    ...
    {
        std::span malformed_headers{StringToBytes("GET / HTTP/1.0\r\nHost=127.0.0.1\r\n\r\n")};
        HTTPRequest req;
    ...
    }
    {
        ...
        std::span incomplete_headers{StringToBytes("GET / HTTP/1.0\r\nHost: ")};
        HTTPRequest req;
    ...
    {
        std::span no_content_length{StringToBytes("GET / HTTP/1.0\r\n\r\n" R"({"method":"getblockcount"})")};
        HTTPRequest req;
    ...
    }
    {
        ...
        std::span bad_content_length{StringToBytes("GET / HTTP/1.0\r\nContent-Length: eleven\r\n\r\n" R"({"method":"getblockcount"})")};
        HTTPRequest req;
    ...
    }
    {
        ...
        std::span excessive_content_length{StringToBytes("GET / HTTP/1.0\r\nContent-Length: 1024\r\n\r\n" R"({"method":"getblockcount"})")};
        HTTPRequest req;
    ...
    }
    {
        ...
        std::span ok_chunked{StringToBytes("GET / HTTP/1.0\n"
    ...
    {
        // Allow (but ignore) Chunk Extensions and Chunk Trailers
        std::span ok_chunked{StringToBytes("GET / HTTP/1.0\n"
                                           "Transfer-Encoding: chunked\n"
                                           "\n"
    ...
    }
    {
        ...
        std::span invalid_chunked{StringToBytes("GET / HTTP/1.0\n"
                                                "Transfer-Encoding: chunked\n"
                                                "\n"
                                                "XVI\n"
                                                R"({"method":"getbl)""\n"
