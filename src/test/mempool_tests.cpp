...
BOOST_AUTO_TEST_CASE(MempoolSizeLimitTest)
{
    ...
    clock += HALFLIFE;
    BOOST_CHECK_EQUAL(pool.GetMinFee(1).GetFeePerK(), Amount{llround((maxFeeRateRemoved.GetFeePerK() + DEFAULT_INCREMENTAL_RELAY_FEE).Int()/2.0)});
    // ... then feerate should drop 1/2 each halflife

    clock += HALFLIFE / 2;
    BOOST_CHECK_EQUAL(pool.GetMinFee(pool.DynamicMemoryUsage() * 5 / 2).GetFeePerK(), Amount{llround((maxFeeRateRemoved.GetFeePerK() + DEFAULT_INCREMENTAL_RELAY_FEE).Int()/4.0)});
    // ... with a 1/2 halflife when mempool is < 1/2 its target size

    clock += HALFLIFE / 4;
    BOOST_CHECK_EQUAL(pool.GetMinFee(pool.DynamicMemoryUsage() * 9 / 2).GetFeePerK(), Amount{llround((maxFeeRateRemoved.GetFeePerK() + DEFAULT_INCREMENTAL_RELAY_FEE).Int()/8.0)});
