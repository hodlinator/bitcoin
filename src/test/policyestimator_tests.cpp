...
BOOST_AUTO_TEST_CASE(BlockPolicyEstimates)
{
    ...
    while (blocknum < 200) {
        ...
        if (blocknum == 3) {
            ...
            BOOST_CHECK(feeEst.estimateFee(1) == CFeeRate(0_sats));
    ...
    for (int i = 1; i < 10;i++) {
        BOOST_CHECK(feeEst.estimateFee(i) == CFeeRate(0_sats) || feeEst.estimateFee(i).GetFeePerK() > origFeeEst[i-1] - deltaFee);
    }
