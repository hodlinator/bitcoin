...
BOOST_FIXTURE_TEST_CASE(handle_missing_inputs, TestChain100Setup)
{
    ...
    CAmount amount_depth_1{(50 * COIN - 1000_sats).AssertValid()};
    CAmount amount_depth_2{(amount_depth_1 - 1000_sats).AssertValid()};
    // Amount for spending coinbase in a 1-in-2-out tx, deducting 1000 in fees
    CAmount amount_split_half{(25 * COIN - 500_sats).AssertValid()};
