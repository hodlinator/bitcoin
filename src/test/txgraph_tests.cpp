...
BOOST_AUTO_TEST_CASE(txgraph_trim_zigzag)
{
    ...
    for (int i = 0; i < NUM_BOTTOM_TX; ++i) {
        graph->AddTransaction(refs.emplace_back(), FeePerWeight{Amount::From(200 - i), 100});
    }
    // Then all top transactions: the i'th top transaction is at position NUM_BOTTOM_TX + i.
    for (int i = 0; i < NUM_TOP_TX; ++i) {
        graph->AddTransaction(refs.emplace_back(), FeePerWeight{Amount::From(100 - i), 100});
    }
...
BOOST_AUTO_TEST_CASE(txgraph_trim_flower)
{
    ...
    for (int i = 0; i < NUM_TOP_TX; ++i) {
        graph->AddTransaction(refs.emplace_back(), FeePerWeight{Amount::From(500 + i), 100});
    }
    graph->SanityCheck();
...
BOOST_AUTO_TEST_CASE(txgraph_trim_huge)
{
    ...
    for (int chain = 0; chain < NUM_TOP_CHAINS; ++chain) {
        for (int chaintx = 0; chaintx < NUM_TX_PER_TOP_CHAIN; ++chaintx) {
            ...
            UAmount fee{UAmount::From(rng.randbits<27>() + 100)};
...
BOOST_AUTO_TEST_CASE(txgraph_trim_big_singletons)
{
    ...
    for (int i = 0; i < NUM_TOTAL_TX; ++i) {
        ...
        const FeePerWeight feerate{Amount::From(500 + i), (i == 88 || i % 20 == 0) ? MAX_CLUSTER_SIZE + 1 : 100};
