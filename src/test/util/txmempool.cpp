...
void MockMempoolMinFee(const CFeeRate& target_feerate, CTxMemPool& mempool)
{
    ...
    mtx.vout.emplace_back(1U * COIN, GetScriptForDestination(WitnessV0ScriptHash(CScript() << OP_TRUE)));
    ...
    const UAmount tx_fee{(target_feerate.GetFee(GetVirtualTransactionSize(*tx)) -
        mempool.m_opts.incremental_relay_feerate.GetFee(GetVirtualTransactionSize(*tx))).AssertToUnsigned()};
    {
