...
struct FeeFrac
{
    ...
    static inline std::pair<int64_t, uint32_t> MulFallback(Amount a, int32_t b) noexcept
    {
    ...
    static inline __int128 Mul(Amount a, int32_t b) noexcept
    {
    ...
    constexpr inline FeeFrac(Amount f, int32_t s) noexcept : fee{f}, size{s} {}
    ...
    Amount EvaluateFee(int32_t at_size) const noexcept
    {
        Assume(size > 0);
        Assume(at_size >= 0);
        if (fee >= 0_sats && fee < 0x200000000_sats) [[likely]] {
            ...
            if constexpr (RoundDown) {
                return Amount::From(static_cast<Amount::inner_type>((uint64_t(fee.Int()) * at_size) / uint32_t(size)));
            } else {
                return Amount::From(static_cast<Amount::inner_type>(CeilDiv(uint64_t(fee.Int()) * at_size, uint32_t(size))));
            }
        } else {
            // Otherwise, use Mul and Div.
            return Amount::From(Div(Mul(fee, at_size), size, RoundDown));
        }
    }

public:
    /** Compute the fee for a given size `at_size` using this object's feerate, rounding down. */
    Amount EvaluateFeeDown(int32_t at_size) const noexcept { return EvaluateFee<true>(at_size); }
    /** Compute the fee for a given size `at_size` using this object's feerate, rounding up. */
    Amount EvaluateFeeUp(int32_t at_size) const noexcept { return EvaluateFee<false>(at_size); }
};
