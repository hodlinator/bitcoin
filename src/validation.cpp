...
private:
    ...
    bool CheckFeeRate(size_t package_size, Amount package_fee, TxValidationState& state) EXCLUSIVE_LOCKS_REQUIRED(::cs_main, m_pool.cs)
    {
        AssertLockHeld(::cs_main);
        AssertLockHeld(m_pool.cs);
        Amount mempoolRejectFee = m_pool.GetMinFee().GetFee(package_size);
...
private:
    ...
    struct SubPackageState {
        ...
        Amount m_total_modified_fees{0_sats};
        ...
        Amount m_conflicting_fees{0_sats};
...
PackageMempoolAcceptResult MemPoolAccept::AcceptMultipleTransactionsInternal(const std::vector<CTransactionRef>& txns, ATMPArgs& args)
{
    ...
    m_subpackage.m_total_modified_fees = std::accumulate(workspaces.cbegin(), workspaces.cend(), Amount{0_sats},
...
UAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    UAmount blockReward = nFees + GetBlockSubsidy(pindex->nHeight, params.GetConsensus());
