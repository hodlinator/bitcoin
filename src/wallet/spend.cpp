                                          ...
                                          const CoinSelectionParams& coin_selection_params)
{
    ...
    Amount selection_target = nTargetValue - pre_set_inputs.GetAppropriateTotal(coin_selection_params.m_subtract_fee_outputs).value_or(0);
    ...
    Amount available_coins_total_amount = available_coins.GetAppropriateTotal(coin_selection_params.m_subtract_fee_outputs).value_or(0);
...
util::Result<SelectionResult> AutomaticCoinSelection(const CWallet& wallet, CoinsResult& available_coins, const Amount& value_to_select, const CoinSelectionParams& coin_selection_params)
{
    ...
    // permissive CoinEligibilityFilter.
    {
        ...
        Amount total_discarded{0};
        Amount total_unconf_long_chain{0};
        ...
        bool sign) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    if (!select_coins_res) {
        ...
        if (available_balance >= recipients_sum) {
            ...
            Amount available_effective_balance = preset_inputs.GetEffectiveTotalAmount().value_or(0) + available_coins.GetEffectiveTotalAmount().value_or(0);
