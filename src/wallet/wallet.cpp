...
UAmount CWallet::GetDebit(const CTxIn &txin) const
{
    LOCK(cs_wallet);
...
UAmount CWallet::GetDebit(const CTransaction& tx) const
{
    UAmount nDebit{0_sats};
    for (const CTxIn& txin : tx.vin)
    {
...
bool CWallet::LoadWalletArgs(std::shared_ptr<CWallet> wallet, const WalletContext& context, bilingual_str& error, std::vector<bilingual_str>& warnings)
{
    ...
    if (const auto arg{args.GetArg("-mintxfee")}) {
        std::optional<UAmount> min_tx_fee = ParseMoney(*arg);
        if (!min_tx_fee) {
    ...
    if (const auto arg{args.GetArg("-maxapsfee")}) {
        ...
        } else if (std::optional<UAmount> max_fee = ParseMoney(max_aps_fee)) {
    ...
    if (const auto arg{args.GetArg("-fallbackfee")}) {
        std::optional<UAmount> fallback_fee = ParseMoney(*arg);
        if (!fallback_fee) {
    ...
    if (const auto arg{args.GetArg("-discardfee")}) {
        std::optional<UAmount> discard_fee = ParseMoney(*arg);
        if (!discard_fee) {
    ...
    if (const auto arg{args.GetArg("-maxtxfee")}) {
        std::optional<UAmount> max_fee = ParseMoney(*arg);
        if (!max_fee) {
    ...
    if (const auto arg{args.GetArg("-consolidatefeerate")}) {
        if (std::optional<UAmount> consolidate_feerate = ParseMoney(*arg)) {
