...
class TestNode():
    ...
    def wait_for_rpc_connection(self, *, wait_for_import=True):
        ...
        poll_per_s = 4
        suppressed_errors: dict[str, int] = collections.defaultdict(int)
        latest_error = ""
    ...
    def stop_node(self, expected_stderr='', *, wait:int=0, wait_until_stopped:bool=True, avoid_exceptions:bool=False):
        """Stop the node.

        Args:
            avoid_exceptions: Stop the node forcefully if necessary, and change
                errors that would otherwise be exceptions into logged warnings.

        Notes:
            - The avoid_exceptions argument should only be set to True when a
            test has already failed, and the caller is already throwing an
            exception of its own, and wants its own exception to take precedence
            over errors from this function. Since the error that happened was
            presumably unknown and unexpected, the node may have already crashed
            or may need to be shut down forcefully.
        """
        if not self.running:
            return
        ...
        if self.rpc_connected:
            ...
            if return_code is None:
                ...
                # error such as FailedToStartError. Set the expected return code to avoid knock-on errors.
                self.expected_ret_code = return_code
        else:
    ...
    def is_node_stopped(self, *, expected_stderr="", expected_ret_code=None):
        ...
        if return_code is None:
            return False
        if not expected_ret_code:
            expected_ret_code = self.expected_ret_code
